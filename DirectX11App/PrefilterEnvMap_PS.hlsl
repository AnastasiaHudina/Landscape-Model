/*Назначение: Генерирует prefiltered environment map для спекулярной части IBL. 
Использует importance sampling с распределением GGX.*/

struct PS_INPUT
{
    float4 position : SV_POSITION;
    float3 localDir : TEXCOORD0;   // как в VS hdrtocubemap
};
...
{
    float3 norm = normalize(input.localDir);
    float3 view = norm;
    ...
    for (uint i = 0u; i < SAMPLE_COUNT; ++i) {
        ...
        // resolution
        //float resolution = 512.0; // изначально
        float resolution = 256.0; // должно совпадать с CUBEMAP_RES
